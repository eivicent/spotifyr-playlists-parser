...
coverage <- compute_data_coverage(daily)
...
# Top artist this week via shared daily CSV reader
...
Days with at least one play vs the calendar span from first to last observed date. Gaps usually mean the daily scrape missed Spotify's short Recently Played window.
...
layout_column_wrap(
  ...
  value_box(
    title    = "Coverage",
    value    = paste0(coverage$coverage_pct, "%"),
    showcase = bsicons::bs_icon("pie-chart"),
    theme    = if (isTRUE(coverage$coverage_pct >= 95)) "success" else if (isTRUE(coverage$coverage_pct >= 85)) "warning" else "danger"
  ),
  value_box(
    title    = "Missing Days",
    value    = scales::comma(coverage$missing_days),
    showcase = bsicons::bs_icon("calendar-x"),
    theme    = "secondary"
  ),
  value_box(
    title    = "Largest Gap",
    value    = paste0(coverage$largest_gap_days, "d"),
    showcase = bsicons::bs_icon("hourglass-split"),
    theme    = "dark"
  ),
  value_box(
    title    = "Last Data Day",
    value    = format(coverage$last_date, "%b %d, %Y"),
